...
AI_FIXTURE_NAMES = [
    path.name for path in sorted((ROOT / "tests" / "fixtures").glob("test-ai-*.png"))
]


@pytest.mark.parametrize("fixture_name", AI_FIXTURE_NAMES)
def test_ai_fixtures_auto_pipeline_returns_metadata_without_crashing(
    fixture_name: str,
) -> None:
...
def test_ai_fixture_two_noisy_pixel_custom_palette_can_reduce_colors() -> None:
    ...
    off = process_image(
        image,
        RestoreSettings(
            algorithm="noisy-pixel-v1", min_scale=1, max_scale=16, palette_cleanup="off"
        ),
    )
    image = Image.open(ROOT / "tests" / "fixtures" / "test-ai-2.png")
    image.thumbnail((256, 256))
    ...
    assert custom.reconstruction["resize_method"] in {
