...
from .resize import (
    downscale_by_dominant_color_cluster,
    downscale_by_majority_vote,
    downscale_by_resampled_grid,
)
...
def process_image(
    ...
    scale, preflight, algorithm_used, detection_metadata = _select_scale_and_algorithm(
        image_array, source_format, settings
    )
    ...
    if (
        ...
        and algorithm_used != preflight.recommended_algorithm
        and algorithm_used != "resampled-grid-v2"
    ):
        warnings.append(
    ...
    if scale.confidence < settings.confidence_threshold:
        warnings.append(
            "Low confidence scale detection; use manual scale override if the result looks wrong."
        )
    ...
    if algorithm_used == "resampled-grid-v2":
        ...
        target_width, target_height = _target_size_from_scale(
            source_size, scale.scale_x, scale.scale_y
        )
        restored = downscale_by_resampled_grid(
            image_array, target_width, target_height, cancel=cancel
        )
        ...
        resize_method = (
            ...
            else "ai-pixel-v2-resampled-cluster"
        )
        target_width, target_height = _target_size_from_scale(
            source_size, scale.scale_x, scale.scale_y
        )
        ai_result = restore_ai_pixel_art(
            image_array,
            target_width,
            target_height,
    ...
    elif algorithm_used == "noisy-pixel-v1" and _uses_fractional_grid(
        ...
        restored = downscale_by_majority_vote(
            image_array,
            int(round(scale.scale_x)),
            int(round(scale.scale_y)),
            cancel=cancel,
    ...
    _report_progress(
        progress, "palette_cleanup", 75.0, "Palette analysis and cleanup..."
    )
    _raise_if_cancelled(cancel)
    ...
    _report_progress(
        progress, "image_reconstruction", 90.0, "Finalizing reconstruction..."
    )
    _raise_if_cancelled(cancel)
    ...
    if settings.original_width is not None or settings.original_height is not None:
        if settings.original_width is None or settings.original_height is None:
            warnings.append(
                "Original size override requires both width and height; it was ignored."
            )
        else:
...
def _resolve_algorithm(settings: RestoreSettings, recommended_algorithm: str) -> str:
    ...
    raise NotImplementedError(
        f"Restore algorithm is not implemented yet: {settings.algorithm}"
    )
...
def _select_scale_and_algorithm(
    ...
    if settings.algorithm != "auto":
        ...
        return (
            scale,
            preflight,
            _resolve_algorithm(settings, preflight.recommended_algorithm),
            _scale_metadata(scale),
        )

    integer_scale = detect_scale(
        image_array, replace(settings, algorithm="integer-grid-v1")
    )
    fractional_scale = detect_scale(
        image_array, replace(settings, algorithm="resampled-grid-v2")
    )
    preflight = analyze_image(
        image_array,
        source_format,
        max(integer_scale.confidence, fractional_scale.confidence),
    )
    algorithm_used = _resolve_algorithm(settings, preflight.recommended_algorithm)

    if algorithm_used == "ai-grid-hypothesis-v1":
        scale = detect_scale(
            image_array, replace(settings, algorithm="ai-grid-hypothesis-v1")
        )
    elif algorithm_used == "integer-grid-v1" and _should_use_fractional(
        integer_scale, fractional_scale
    ):
        algorithm_used = "resampled-grid-v2"
        scale = fractional_scale
    elif (
        algorithm_used == "noisy-pixel-v1"
        and fractional_scale.confidence >= integer_scale.confidence
    ):
        scale = fractional_scale
    else:
        scale = integer_scale
    ...
    metadata = {
        "integer": _scale_metadata(integer_scale),
        "fractional": _scale_metadata(fractional_scale),
    }
    if algorithm_used == "ai-grid-hypothesis-v1":
...
def _should_use_fractional(
    integer_scale: ScaleEstimate, fractional_scale: ScaleEstimate
) -> bool:
    if fractional_scale.confidence < 0.2:
        return False
    if fractional_scale.confidence > integer_scale.confidence + 0.05:
        return True
    return integer_scale.confidence < 0.2 and _uses_fractional_grid(
        fractional_scale.scale_x, fractional_scale.scale_y
    )
...
def _target_size_from_scale(
    source_size: tuple[int, int], scale_x: float, scale_y: float
) -> tuple[int, int]:
