...
def analyze_image(
    image_array: np.ndarray, source_format: str | None, grid_confidence: float
) -> PreflightAnalysis:
    ...
    near_duplicate_ratio = _near_duplicate_color_ratio(
        unique_color_count, estimated_palette_size
    )
    ...
    ai_score = _ai_artifact_score(
        source_format, noise_score, near_duplicate_ratio, grid_confidence
    )
    ai_pixel_v2_score = _ai_pixel_v2_score(
        source_format, ai_score, near_duplicate_ratio, grid_confidence
    )
    recommended_algorithm, confidence, reasons = _recommend_algorithm(
        jpeg_score, ai_score, ai_pixel_v2_score, grid_confidence
    )
...
def _near_duplicate_color_ratio(
    unique_color_count: int, estimated_palette_size: int
) -> float:
...
def _noise_score(
    image_array: np.ndarray, unique_color_count: int, estimated_palette_size: int
) -> float:
    pixel_count = max(1, image_array.shape[0] * image_array.shape[1])
    unique_density = min(1.0, unique_color_count / min(pixel_count, 4096))
    palette_fragmentation = _near_duplicate_color_ratio(
        unique_color_count, estimated_palette_size
    )
    edge_noise = _edge_noise_score(image_array)
    return _clamp01(
        (unique_density * 0.35) + (palette_fragmentation * 0.45) + (edge_noise * 0.20)
    )
...
def _edge_noise_score(image_array: np.ndarray) -> float:
    ...
    weak_edges = np.count_nonzero((dx > 2) & (dx < 28)) + np.count_nonzero(
        (dy > 2) & (dy < 28)
    )
...
def _jpeg_artifact_score(
    source_format: str | None, noise_score: float, near_duplicate_ratio: float
) -> float:
...
def _ai_artifact_score(
    ...
    non_jpeg_bias = (
        0.15 if (source_format or "").upper() not in ("JPEG", "JPG") else 0.0
    )
    uncertain_grid = 1.0 - _clamp01(grid_confidence)
    return _clamp01(
        non_jpeg_bias
        + (noise_score * 0.45)
        + (near_duplicate_ratio * 0.25)
        + (uncertain_grid * 0.15)
    )


def _ai_pixel_v2_score(
    source_format: str | None,
    ai_score: float,
    near_duplicate_ratio: float,
    grid_confidence: float,
    ...
    uncertain_grid = 1.0 - _clamp01(grid_confidence)
    return _clamp01(
        (ai_score * 0.55) + (near_duplicate_ratio * 0.30) + (uncertain_grid * 0.15)
    )


def _recommend_algorithm(
    jpeg_score: float, ai_score: float, ai_pixel_v2_score: float, grid_confidence: float
