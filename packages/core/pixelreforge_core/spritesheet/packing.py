...
from .models import AtlasFrame, AtlasSizeError, SpriteFrame, SpriteSheetError, SpriteSheetSettings
...
def pack_frames(
    ...
    *,
    ...
    if settings.packing_mode == "grid":
        ...
        placements, output_size = _pack_compact(frames, ordered_indices, settings, cancel)
...
def _pack_grid(
    ...
    count = len(frames)
    columns = settings.grid_columns or _best_grid_columns(count, cell_width, cell_height, settings)
    rows = ceil(count / columns)
    required_width = _grid_extent(columns, cell_width, settings.padding, settings.border_padding)
    required_height = _grid_extent(rows, cell_height, settings.padding, settings.border_padding)
    ...
    if required_width > output_size[0] or required_height > output_size[1]:
        raise AtlasSizeError("Sprites do not fit in the requested grid atlas dimensions.")
...
def _best_grid_columns(count: int, cell_width: int, cell_height: int, settings: SpriteSheetSettings) -> int:
    candidates: list[tuple[int, int]] = []
    for columns in range(1, count + 1):
        rows = ceil(count / columns)
        width = _grid_extent(columns, cell_width, settings.padding, settings.border_padding)
        height = _grid_extent(rows, cell_height, settings.padding, settings.border_padding)
        try:
...
def _pack_compact(
    ...
    if settings.atlas_width is not None and settings.atlas_height is not None:
        _resolve_output_size(settings.atlas_width, settings.atlas_height, settings)
    content_sizes = [_minimum_content_size(frames[index], settings) for index in ordered_indices]
    outer_sizes = [_outer_size(frames[index], settings, False) for index in ordered_indices]
    ...
    candidates = _compact_candidates(minimum_width, minimum_height, total_area, settings)
    ...
    for inner_width, inner_height in candidates:
        ...
        if placements is None:
            continue
        used_width = max(placement.x + placement.content_width for placement in placements) + settings.border_padding
        used_height = max(placement.y + placement.content_height for placement in placements) + settings.border_padding
        output_size = _resolve_output_size(used_width, used_height, settings)
        if output_size[0] > inner_width + 2 * settings.border_padding or output_size[1] > inner_height + 2 * settings.border_padding:
            continue
    ...
    if best is None:
        raise AtlasSizeError("Sprites do not fit within the configured atlas dimensions.")
    return best[1], best[2]


def _compact_candidates(minimum_width: int, minimum_height: int, total_area: int, settings: SpriteSheetSettings) -> list[tuple[int, int]]:
    ...
    if settings.power_of_two:
        widths = [value - 2 * border for value in _power_of_two_values(minimum_width + 2 * border, settings.max_width)]
        heights = [value - 2 * border for value in _power_of_two_values(minimum_height + 2 * border, settings.max_height)]
    else:
        ...
        heights = _dimension_candidates(minimum_height, max_inner_height, preferred_height)
    ...
    if settings.force_square:
        ...
        return [
            (side, side)
            for side in sides
            if side <= max_inner_width and side <= max_inner_height and (side + 2 * border) ** 2 <= settings.max_pixels
        ]
    return sorted(
        (
            (width, height)
...
def _maxrects_pack(
    ...
    *,
    ...
    if width < 1 or height < 1:
        return None
    free_rectangles = [_Rect(settings.border_padding, settings.border_padding, width, height)]
    placements: list[_Placement] = []
    ...
    for rotated in orientations:
        ...
        for rect in free_rectangles:
            if occupied_width > rect.width or occupied_height > rect.height:
                continue
            if rect.x + content_width > content_right_limit or rect.y + content_height > content_bottom_limit:
                continue
            horizontal = rect.width - occupied_width
            vertical = rect.height - occupied_height
            score = (min(horizontal, vertical), max(horizontal, vertical), rect.y, rect.x, int(rotated))
...
def _split_free_rectangles(free_rectangles: Sequence[_Rect], occupied: _Rect) -> list[_Rect]:
    split: list[_Rect] = []
    for free in free_rectangles:
        ...
        if occupied.right < free.right:
            split.append(_Rect(occupied.right, free.y, free.right - occupied.right, free.height))
        if occupied.y > free.y:
            split.append(_Rect(free.x, free.y, free.width, occupied.y - free.y))
        if occupied.bottom < free.bottom:
            split.append(_Rect(free.x, occupied.bottom, free.width, free.bottom - occupied.bottom))
...
def _outer_size(frame: SpriteFrame, settings: SpriteSheetSettings, rotated: bool) -> tuple[int, int]:
...
def _minimum_content_size(frame: SpriteFrame, settings: SpriteSheetSettings) -> tuple[int, int]:
...
def _resolve_output_size(required_width: int, required_height: int, settings: SpriteSheetSettings) -> tuple[int, int]:
    if settings.atlas_width is not None and settings.atlas_height is not None:
    ...
    if settings.power_of_two and (not _is_power_of_two(width) or not _is_power_of_two(height)):
        raise AtlasSizeError("Power-of-two output requires power-of-two atlas dimensions.")
...
def _extruded_image(image: Image.Image, amount: int) -> Image.Image:
    ...
    expanded = Image.new("RGBA", (width + 2 * amount, height + 2 * amount), (0, 0, 0, 0))
    expanded.alpha_composite(image, (amount, amount))
    nearest = Image.Resampling.NEAREST
    expanded.alpha_composite(image.crop((0, 0, width, 1)).resize((width, amount), nearest), (amount, 0))
    expanded.alpha_composite(image.crop((0, height - 1, width, height)).resize((width, amount), nearest), (amount, amount + height))
    expanded.alpha_composite(image.crop((0, 0, 1, height)).resize((amount, height), nearest), (0, amount))
    expanded.alpha_composite(image.crop((width - 1, 0, width, height)).resize((amount, height), nearest), (amount + width, amount))
    expanded.alpha_composite(image.crop((0, 0, 1, 1)).resize((amount, amount), nearest), (0, 0))
    expanded.alpha_composite(image.crop((width - 1, 0, width, 1)).resize((amount, amount), nearest), (amount + width, 0))
    expanded.alpha_composite(image.crop((0, height - 1, 1, height)).resize((amount, amount), nearest), (0, amount + height))
    expanded.alpha_composite(
        image.crop((width - 1, height - 1, width, height)).resize((amount, amount), nearest),
...
def _ordered_indices(frames: Sequence[SpriteFrame], sort_mode: str) -> list[int]:
    ...
    if sort_mode == "width":
        return sorted(indices, key=lambda index: (-frames[index].image.width, -frames[index].image.height, frames[index].name.casefold()))
    if sort_mode == "height":
        return sorted(indices, key=lambda index: (-frames[index].image.height, -frames[index].image.width, frames[index].name.casefold()))
    return sorted(
        indices,
        key=lambda index: (-frames[index].image.width * frames[index].image.height, -frames[index].image.height, -frames[index].image.width, frames[index].name.casefold()),
    )
